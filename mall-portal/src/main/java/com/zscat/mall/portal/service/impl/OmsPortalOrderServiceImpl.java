...
import com.zscat.common.exception.ApiRRException;
...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
    ...
    @Override
    public ConfirmOrderResult generateConfirmOrder(UmsMember currentMember) {
     ...
     //   result.setCouponHistoryDetailList(couponHistoryDetailList);
        ...
        //获取积分使用规则
        UmsIntegrationConsumeSetting integrationConsumeSetting = integrationConsumeSettingService.selectById(1L);
    ...
    @Override
    public CommonResult generateOrder(OrderParam orderParam,UmsMember currentMember) {
        ...
        if( "1".equals(type)) {
            ...
            if (!CollectionUtils.isEmpty(list)) {
                cartPromotionItemList = promotionService.calcCartPromotion(list);
            }
        ...
        //收货人信息：姓名、电话、邮编、地址
        UmsMemberReceiveAddress address = memberReceiveAddressService.getItem(orderParam.getMemberReceiveAddressId(),currentMember);
        ...
        //    push(currentMember,order,orderParam.getPage(),orderParam.getFormId());
        }
    ...
    public CommonResult paySuccess(Long orderId) {
        ...
        int count = orderService.updateSkuStock(orderDetail.getOrderItemList());
     ...
     *
     */
   /* public void push(UmsMember umsMember, OmsOrder order,String page,String formId) {
        ...
        try {
            accessToken = wechatApiService.getAccessToken("health_applet");

            OrderItem oderIterm = orderItemDao.findByOrderId(orderId);

            String templateId = ConfigUtil.getString("order.exchange.applet.template", "g53EPsIbZu4U0M48Y4pmr1wSvyBB_w8v96TifCQGllg");
            ...
            param.put("keyword2", new TemplateData(DateUtil.dateToStr(order.getOrderTime(),"yyyy-MM-dd HH:mm:ss"), "#EE0000"));
            param.put("keyword3", new TemplateData(Validator.doubleTrans(Validator.rountTwo(order.getPayAmount().doubleValue()))+"海贝", "#EE0000"));
            param.put("keyword4", new TemplateData(order.getProvince()+" "+
                    order.getCity()+" "+order.getDistrict()+" "+order.getAddress(), "#EE0000"));
        ...
        //调用发送微信消息给用户的接口    ********这里写自己在微信公众平台拿到的模板ID
            WX_TemplateMsgUtil.sendWechatMsgToUser(healthUser.getOpenid(), templateId, page+"?orderId="+order.getId(),
    ...
    public CommonResult cancelTimeOutOrder() {
        OmsOrderSetting orderSetting = orderSettingService.getItem(1L);
        //查询超时、未支付的订单及订单详情
        List<OmsOrderDetail> timeOutOrders = orderService.getTimeOutOrders(orderSetting.getNormalOrderOvertime());
        ...
        orderService.updateOrderStatus(ids, 4);
        for (OmsOrderDetail timeOutOrder : timeOutOrders) {
            //解除订单商品库存锁定
            orderService.releaseSkuStockLock(timeOutOrder.getOrderItemList());
            //修改优惠券使用状态
    ...
    public void cancelOrder(Long orderId) {
        ...
        List<OmsOrder> cancelOrderList = orderService.selectByExample(example);
     ...
     */
    @Override
    public ConfirmOrderResult submitPreview(Map<String, Object> params,UmsMember currentMember) {
    ...
    public int payOrder(TbThanks tbThanks) {
        ...
        //设置订单为已付款
        OmsOrder tbOrder=orderService.selectByPrimaryKey(tbThanks.getOrderId());
        if (tbOrder==null){
        ...
        if(orderService.updateByPrimaryKeySelective(tbOrder)!=1){
            throw new ApiRRException("更新订单失败");
        }
        //恢复所有下单商品的锁定库存，扣减真实库存
        OmsOrderDetail orderDetail = orderService.detail(tbOrder.getId());
        int count = orderService.updateSkuStock(orderDetail.getOrderItemList());
        //发送通知确认邮件
    ...
    private void updateCouponStatus(Long couponId, Long memberId, Integer useStatus) {
        ...
        List<SmsCouponHistory> couponHistoryList = couponHistoryService.selectByExample(example);
        if (!CollectionUtils.isEmpty(couponHistoryList)) {
            SmsCouponHistory couponHistory = couponHistoryList.get(0);
            couponHistory.setUseTime(new Date());
            couponHistory.setUseStatus(useStatus);
            couponHistoryService.updateByPrimaryKeySelective(couponHistory);
        }
    }
    ...
    private SmsCouponHistoryDetail getUseCoupon(List<CartPromotionItem> cartPromotionItemList, Long couponId) {
        List<SmsCouponHistoryDetail> couponHistoryDetailList = null;
                //memberCouponService.listCart(cartPromotionItemList, 1);
    ...
    private void lockStock(List<CartPromotionItem> cartPromotionItemList) {
        for (CartPromotionItem cartPromotionItem : cartPromotionItemList) {
            PmsSkuStock skuStock = skuStockService.selectById(cartPromotionItem.getProductSkuId());
